...
// commands
#define KM_LCD_CLEARDISPLAY 0x01
#define KM_LCD_RETURNHOME 0x02
#define KM_LCD_ENTRYMODESET 0x04
#define KM_LCD_DISPLAYCONTROL 0x08
#define KM_LCD_CURSORSHIFT 0x10
#define KM_LCD_FUNCTIONSET 0x20
#define KM_LCD_SETCGRAMADDR 0x40
#define KM_LCD_SETDDRAMADDR 0x80

// flags for display entry mode
#define KM_LCD_ENTRYRIGHT 0x00
#define KM_LCD_ENTRYLEFT 0x02
#define KM_LCD_ENTRYSHIFTINCREMENT 0x01
#define KM_LCD_ENTRYSHIFTDECREMENT 0x00

// flags for display on/off control
#define KM_LCD_DISPLAYON 0x04
#define KM_LCD_DISPLAYOFF 0x00
#define KM_LCD_CURSORON 0x02
#define KM_LCD_CURSOROFF 0x00
#define KM_LCD_BLINKON 0x01
#define KM_LCD_BLINKOFF 0x00

// flags for display/cursor shift
#define KM_LCD_DISPLAYMOVE 0x08
#define KM_LCD_CURSORMOVE 0x00
#define KM_LCD_MOVERIGHT 0x04
#define KM_LCD_MOVELEFT 0x00

// flags for function set
#define KM_LCD_8BITMODE 0x10
#define KM_LCD_4BITMODE 0x00
#define KM_LCD_2LINE 0x08
#define KM_LCD_1LINE 0x00

// flags for back light control
#ifndef KM_USE_ATB
#define KM_LCD_BACKLIGHT   _BV(PIN3)
#else
#define KM_LCD_BACKLIGHT   _BV(PIN7)
#endif
#define KM_LCD_NOBACKLIGHT 0x00


#define KM_LCD_PIN_EN PIN2
#define KM_LCD_PIN_RW PIN1
#define KM_LCD_PIN_RS PIN0

#define En _BV(KM_LCD_PIN_EN)  // Enable byte value
#define Rw _BV(KM_LCD_PIN_RW)  // Read/Write byte value
#define Rs _BV(KM_LCD_PIN_RS)  // Register select byte value
...
void lcdInit(uint8_t lcdCols, uint8_t lcdRows, uint8_t charSize) {
    ...
    _rows = lcdRows;
    _charSize = charSize;
    _backlightVal = KM_LCD_BACKLIGHT;
...
void lcdBegin(void) {
    ...
    KM_LCD_DDR = 0xFF;
    KM_LCD_PORT = 0x00;
    _displayFunction = KM_LCD_4BITMODE | KM_LCD_1LINE | LCD_5x8DOTS;

    if (_rows > 1) {
        _displayFunction |= KM_LCD_2LINE;
    }
    ...
    _backlightVal = KM_LCD_NOBACKLIGHT;
    ...
    lcdCommand(KM_LCD_FUNCTIONSET | _displayFunction);
    // turn the display on with no cursor or blinking default
    _displayControl = KM_LCD_DISPLAYON | KM_LCD_CURSOROFF | KM_LCD_BLINKOFF;
    lcdDisplay();
    // clear it off
    lcdClear();
    // Initialize to default text direction (for roman languages)
    _displayMode = KM_LCD_ENTRYLEFT | KM_LCD_ENTRYSHIFTDECREMENT;
    // set the entry mode
    lcdCommand(KM_LCD_ENTRYMODESET | _displayMode);
    lcdHome();
}

void lcdClear() {
    lcdCommand(KM_LCD_CLEARDISPLAY);// clear display, set cursor position to zero
...
void lcdHome() {
    lcdCommand(KM_LCD_RETURNHOME);  // set cursor position to zero
    _spaceToEndOfTheLine = _cols;
    _delay_ms(2);  // this command takes a long time!
}

void lcdNoDisplay() {
    _displayControl &= ~KM_LCD_DISPLAYON;
    lcdCommand(KM_LCD_DISPLAYCONTROL | _displayControl);
}
...
void lcdNoBlink() {
    _displayControl &= ~KM_LCD_BLINKON;
    lcdCommand(KM_LCD_DISPLAYCONTROL | _displayControl);
}
...
void lcdNoCursor() {
    _displayControl &= ~KM_LCD_CURSORON;
    lcdCommand(KM_LCD_DISPLAYCONTROL | _displayControl);
}
...
void lcdScrollDisplayLeft() {
    lcdCommand(KM_LCD_CURSORSHIFT | KM_LCD_DISPLAYMOVE | KM_LCD_MOVELEFT);
}

void lcdScrollDisplayRight() {
    lcdCommand(KM_LCD_CURSORSHIFT | KM_LCD_DISPLAYMOVE | KM_LCD_MOVERIGHT);
}

void lcdLeftToRight() {
    _displayMode |= KM_LCD_ENTRYLEFT;
    lcdCommand(KM_LCD_ENTRYMODESET | _displayMode);
}
...
void lcdAutoscroll() {
    _displayMode |= KM_LCD_ENTRYSHIFTINCREMENT;
    lcdCommand(KM_LCD_ENTRYMODESET | _displayMode);
}
...
void lcdCreateChar(uint8_t location , const uint8_t charMap[]) {
    ...
    lcdCommand(KM_LCD_SETCGRAMADDR | (location << 3));
    for (int i=0; i < 8; i++) {
        lcdWrite(charMap[i]);
    }
}
...
void lcdSetCursor(uint8_t col, uint8_t row) {
    ...
    lcdCommand(KM_LCD_SETDDRAMADDR | (col + row_offsets[row]));
...
#ifndef KM_USE_ATB
void lcdPortWrite(uint8_t data) {
    KM_LCD_PORT = data | _backlightVal;
}
#else
void lcdPortWrite(uint8_t data) {
        uint8_t newData = (data & 0xF0) >> 1;
        newData |= data &0x0F;
        KM_LCD_PORT = newData | _backlightVal;
}
#endif